...
SYSTEM_PROJECT_DIR = PROJECT_DIR
DEMO_MODE = os.environ.get('DEMO_MODE', 'true').lower() in {'1', 'true', 'yes', 'on'}
DEMO_ALLOW_SETUP = os.environ.get('DEMO_ALLOW_SETUP', 'false').lower() in {'1', 'true', 'yes', 'on'}
DEMO_MAX_USERS = int(os.environ.get('DEMO_MAX_USERS', '20'))
DEMO_MAX_UPLOAD_FILES_PER_USER = int(os.environ.get('DEMO_MAX_UPLOAD_FILES_PER_USER', '20'))
DEMO_RUNTIME_DIR = os.environ.get('DEMO_RUNTIME_DIR') or os.path.join(SYSTEM_PROJECT_DIR, 'runtime')
...
def _safe_username(username: str) -> str:
    safe = re.sub(r'[^\w.-]+', '-', (username or '').strip()).strip('.-')
    return safe or 'anonymous'


def _sync_project_modules(project_dir: str) -> None:
    for module_name in ('auto_ingest', 'graph'):
        module = sys.modules.get(module_name)
        if module is not None:
            setattr(module, 'PROJECT_DIR', project_dir)
            setattr(module, 'WIKI_DIR', os.path.join(project_dir, 'wiki'))


def _set_active_project_dir(project_dir: str) -> None:
    global PROJECT_DIR, WIKI_DIR, RAW_DIR
    PROJECT_DIR = project_dir
    WIKI_DIR = os.path.join(project_dir, 'wiki')
    RAW_DIR = os.path.join(project_dir, 'raw', 'sources')
    _sync_project_modules(project_dir)
...
def _ensure_demo_workspace(username: str) -> str:
    project_dir = os.path.join(DEMO_RUNTIME_DIR, 'users', _safe_username(username))
    os.makedirs(project_dir, exist_ok=True)
    for filename in ('schema.md', 'purpose.md'):
        _copy_if_missing(os.path.join(SYSTEM_PROJECT_DIR, filename), os.path.join(project_dir, filename))
    _copy_if_missing(os.path.join(SYSTEM_PROJECT_DIR, 'config'), os.path.join(project_dir, 'config'))
    _copy_if_missing(os.path.join(SYSTEM_PROJECT_DIR, 'wiki', 'templates'), os.path.join(project_dir, 'wiki', 'templates'))
    _set_active_project_dir(project_dir)
    init_wiki()
    _set_active_project_dir(project_dir)
    return project_dir


def _select_system_project_dir() -> None:
    _set_active_project_dir(SYSTEM_PROJECT_DIR)


def _is_explicit_project_override() -> bool:
    """Honor tests and explicit callers that patch PROJECT_DIR/WIKI_DIR."""
    try:
        project = Path(PROJECT_DIR).resolve()
        system = Path(SYSTEM_PROJECT_DIR).resolve()
        runtime_users = (Path(DEMO_RUNTIME_DIR) / 'users').resolve()
        if project == system:
            return False
        try:
            if project.is_relative_to(runtime_users):
                return False
        except AttributeError:
            if str(project).startswith(str(runtime_users)):
                return False
        return True
    except Exception:
        return False
...
def _demo_user_count() -> int:
    try:
        from auth import _load_users
        return len(_load_users(SYSTEM_PROJECT_DIR))
    except Exception:
        return 0


def _demo_uploaded_count(project_dir: str) -> int:
    ...
    try:
        ...
        return sum(len(batch.get('original_files') or []) for batch in batches if isinstance(batch, dict))
    except Exception:
        return 0


def _demo_quota(project_dir: str) -> Dict:
    used = _demo_uploaded_count(project_dir)
    limit = DEMO_MAX_UPLOAD_FILES_PER_USER
    return {
        'mode': 'cloud_demo',
        'limit': limit,
        'used': used,
...
@app.before_request
def _select_demo_workspace():
    if not DEMO_MODE:
        return None
    if _is_explicit_project_override():
        _sync_project_modules(PROJECT_DIR)
        return None
    from auth import current_username
    path = request.path or ''
    if path.startswith('/api/auth/') or path.startswith('/api/admin/') or path in _AUTH_PUBLIC_EXACT or path.startswith('/assets/'):
        _select_system_project_dir()
        return None
    user = current_username()
    if user:
        _ensure_demo_workspace(user)
    else:
        _select_system_project_dir()
    return None
...
def api_auth_status():
    ...
    return jsonify({
        ...
        'demo': {
            ...
            'max_upload_files_per_user': DEMO_MAX_UPLOAD_FILES_PER_USER,
            'allow_setup': DEMO_ALLOW_SETUP,
        },
    })
...
def api_auth_setup():
    ...
    if DEMO_MODE and not DEMO_ALLOW_SETUP:
        return jsonify({'error': '云端体验版已关闭公开初始化，请由管理员预置账号或邀请码'}), 403
...
def api_auth_register():
    ...
    data = request.json or {}
    if DEMO_MODE and _demo_user_count() >= DEMO_MAX_USERS:
        return jsonify({'error': f'体验版注册名额已满，最多支持 {DEMO_MAX_USERS} 个用户'}), 403
    try:
        user = register_user(
            PROJECT_DIR,
...
@app.route('/api/demo/quota', methods=['GET'])
def api_demo_quota():
    from auth import current_username
    user = current_username()
    if not user:
        return jsonify({'error': 'unauthorized'}), 401
    project_dir = _ensure_demo_workspace(user) if DEMO_MODE else PROJECT_DIR
    return jsonify(_demo_quota(project_dir))
...
def api_stats():
    ...
    recent_notes.sort(key=lambda x: (x.get('created') or x.get('updated') or ''), reverse=True)
...
def api_ingest_upload():
    ...
    if DEMO_MODE:
        quota = _demo_quota(PROJECT_DIR)
        if len(files) > quota['remaining']:
            return jsonify({
                'error': f"体验版每个账号最多上传 {quota['limit']} 个文件，当前已用 {quota['used']} 个，剩余 {quota['remaining']} 个",
                'demo_quota': quota,
            }), 403
    ...
    if DEMO_MODE:
        result['demo_quota'] = _demo_quota(PROJECT_DIR)
