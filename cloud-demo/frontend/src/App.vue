              ...
              <section class="home-panel">
                ...
                <div class="qa-list">
                  ...
                  >
                    ...
                    <div class="qa-date">{{ note.created || note.updated }}</div>
                  </button>
                  <div v-if="!homeStats.recent_notes?.length" class="detail-item">最近还没有新建笔记</div>
                </div>
              </section>
            </div>
          ...
          <template v-else-if="activeView === 'ingest'">
            ...
            <div class="workbench-grid ingest-workbench">
              <div class="upload-panel">
                ...
                <div class="demo-cloud-notice">
                  <strong>云端体验版</strong>
                  <span>上传内容会暂存在服务器用于试用；正式使用建议本地部署，数据保存在本机。</span>
                  <em v-if="demoQuota">已用 {{ demoQuota.used }} / {{ demoQuota.limit }} 个文件，剩余 {{ demoQuota.remaining }} 个</em>
                </div>
                <el-upload
                  drag
                  multiple
                ...
                <el-button
                  ...
                  size="large"
                  :loading="uploading"
                  :disabled="!fileList.length || uploadOverDemoQuota"
...
const uploadOverDemoQuota = computed(() => {
  if (!demoQuota.value) return false
  return fileList.value.length > demoQuota.value.remaining
})
...
async function refreshAll() {
  await Promise.all([loadBatches(), loadPages(), loadConfig(), loadAgentStatus(), loadDemoQuota()])
  if (activeView.value === 'graph') {
    await loadGraph()
  }
}

async function loadDemoQuota() {
  try {
    demoQuota.value = await api('/api/demo/quota')
  } catch {
    demoQuota.value = null
  }
}
...
async function uploadFiles() {
  ...
  if (uploadOverDemoQuota.value) {
    ElMessage.warning(`体验版每个账号最多上传 ${demoQuota.value.limit} 个文件，当前剩余 ${demoQuota.value.remaining} 个`)
    return
  }
  ...
  try {
    ...
    if (result.demo_quota) {
      demoQuota.value = result.demo_quota
    }
    fileList.value = []
    uploadFilesExpanded.value = false
